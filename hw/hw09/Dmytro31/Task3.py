...
# Function to get weather details from OWM API
def get_weather():
    try:
        owm = OWM(API_KEY)
        ...
        weather_details = f"Weather in {location}:\n"
        weather_details += f"Status: {w.detailed_status}\n"
        weather_details += f"Wind: {w.wind()}\n"
        ...
        label.config(text="Error: Could not fetch weather data.")
